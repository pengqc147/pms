...
<script type="application/javascript">
	$(function () {
		$.ajax({
			type:"get",
			url:"${pageContext.request.contextPath}/notice/getNewList",
			success:function (msg) {
				if(msg.map.statusCode == 200){
				    $(msg.map.list).each(function (index, item) {
						var li = "<li onclick='showWindows("+item.nid+")' class=\"ue-clear\">\n" +
                            "\t\t\t\t\t<span>"+moment(item.ndate).format('YYYY-MM-DD')+"</span>&nbsp;&nbsp;&nbsp;<a href=\"#\"class=\"notice-title\">"+item.ntitle+"</a>\n" +
                            "\t\t\t\t\t</li>\n" +
                            "\t\t\t\t\t<p>"
                        $("#notice-show").append(li);
                    });
                }
            }
		})
    });
	function showWindows(nid) {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/notice/getThisList/"+nid,
			success:function (msg) {
...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	<!-- 标题 -->
	<div id="ntitle" style="background: #F8F7F7; width: 100%; height: 2rem; font-size: 1.5rem;font-weight: bold; line-height: 2rem; border: 1px solid #999; text-align: center;" >

	</div>
	<!-- 内容 -->
	<div id="content" style="text-indent: 50px; height: 4rem; font-size: 1.5rem;font-weight: bold; padding: 0.5rem; line-height: 4rem; ">
	...
	<table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr bgcolor="#FFFFFF">
			<td>
				<ul id="notice-show" class="notice-list">
				</ul>
			</td>
		</tr>
	</table>
